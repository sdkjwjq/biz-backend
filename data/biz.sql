...
-- 4.4 绩效手动填报审核单
DROP TABLE IF EXISTS `biz_performance_audit_log`;
DROP TABLE IF EXISTS `biz_performance_audit_snapshot`;
DROP TABLE IF EXISTS `biz_performance_submission`;
CREATE TABLE `biz_performance_submission` (
...
CREATE TABLE `biz_performance_audit_snapshot` (
...
CREATE TABLE `biz_performance_audit_log` (
...
-- 3.2 审批状态快照
DROP TABLE IF EXISTS `biz_audit_snapshot`;
CREATE TABLE `biz_audit_snapshot` (
    `snapshot_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '快照ID',
    `sub_id` bigint(20) NOT NULL COMMENT '提交ID',
    `target_type` varchar(20) NOT NULL COMMENT '对象类型 TASK/LEVEL4_TASK',
    `target_id` bigint(20) NOT NULL COMMENT '任务ID或四级任务ID',
    ...
    `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`snapshot_id`),
    UNIQUE KEY `uk_audit_snapshot_target` (`sub_id`, `target_type`, `target_id`),
    KEY `idx_audit_snapshot_sub` (`sub_id`),
    CONSTRAINT `fk_snapshot_submission` FOREIGN KEY (`sub_id`) REFERENCES `biz_material_submission` (`sub_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='审批提交前状态快照表';
