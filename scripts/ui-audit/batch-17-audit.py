...
assert sql('SELECT COUNT(*) FROM sys_notice WHERE to_user_id=910002') == '0'
results.append({'id':'17-1','monthly':monthly,'annual':annual,'validLeaderNoticeCount':0})
...
monthly = call('/scheduled/month_leader_trigger', admin, {})
annual = call('/scheduled/year_trigger', admin, {})
notices = [json.loads(row) for row in sql("SELECT JSON_OBJECT('title',title,'content',content) FROM sys_notice WHERE to_user_id=910002 ORDER BY notice_id").splitlines()]
...
assert sql('SELECT COUNT(*) FROM biz_trend_data') == '0'
sql('UPDATE biz_task SET current_value=0 WHERE task_id=930002')
control = call('/dashboard/trend/record', admin, {})
assert control == '手动记录成功'
assert sql('SELECT COUNT(*) FROM biz_trend_data') == '1'
results.append({'id':'17-3','withNullValue':failed,'failedRecordCount':0,'afterZeroValue':control,'controlRecordCount':1})
output = ROOT / 'target/ui-audit/evidence-batch-17'
output.mkdir(parents=True, exist_ok=True)
(output / 'reminder-trend.json').write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding='utf-8')
