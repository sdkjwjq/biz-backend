...
async function main() {
  ...
  try {
    ...
    assert.equal(Number(await page.getByRole('menuitem', { name: /审核中心/ }).locator('.el-badge__content').innerText()), pending.length);
    await page.screenshot({ path: path.join(root, 'evidence-fixed/badge.png'), fullPage: true });
    await fs.writeFile(path.join(root, 'evidence-fixed/badge.json'), JSON.stringify({ pendingCount: pending.length, sidebarBadgeCount: badgeCount }, null, 2) + '\n');
