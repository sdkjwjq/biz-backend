...
const output = path.resolve(__dirname, '../../target/ui-audit/evidence-fixed-sweep');
  ...
  try {
    ...
    for (const id of [110228, 910001, 910003, 1910001]) {
      ...
      for (const route of routes) {
        ...
        await page.locator('.common-layout, .dashboard-fixed-container').first().waitFor({ timeout: 20000 });
      ...
      if (id === 110228) {
        ...
        await page.waitForURL(url => url.pathname === '/home/works');
        assert.equal(new URL(page.url()).pathname, '/home/works');
      }
      await context.close();
    }
    const routes = ['/system/allUsers','/system/dept/920001','/system/notice','/biz/tasks','/biz/tasks/930002',
      ...
      '/dashboard/completion/year?year=2026','/dashboard/completion/midterm?year=2028',
    ...
    assert.equal(stats.inProgressCount, stats.totalTasks);
    ...
    assert.ok(uploaded[1].body.fileId);
    result.findings.push({ id: 'uppercase-upload', uploaded });
    assert.deepEqual(result.api.filter(row => row.status !== 200 || row.businessError), []);
