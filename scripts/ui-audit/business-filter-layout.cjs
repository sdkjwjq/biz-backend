...
(async () => {
  ...
  try {
    ...
    for (const [route, title, owner, pending, inactive] of [
      ...
      await expect(bar.getByRole('combobox', { name: title + '当前处理部门', exact: true })).toHaveCount(0);
      ...
      await page.evaluate(route => {
        const suffix = localStorage.getItem('token').split('.')[2];
        const key = `works_browse_v1_${route === 'achievement' ? 'achievements' : 'performance'}:${suffix}`;
        const saved = JSON.parse(sessionStorage.getItem(key));
        saved.fields.handler = '999999';
      ...
      await expect(page.getByRole('row').filter({ hasText: inactive })).toBeVisible();
      checks.push(title + ': owning department, range, memory and reset; removed handler and ignored legacy handler cache');
      if (title === '绩效') {
