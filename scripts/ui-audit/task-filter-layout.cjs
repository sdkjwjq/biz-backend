...
(async () => {
  ...
  try {
    ...
    await expect(bar.locator('.advanced-inline')).toBeVisible();
    await expect(bar.getByRole('button', { name: /更多筛选/ })).toBeHidden();
    ...
    assert.ok(Math.max(...boxes.map(b => b.y)) - Math.min(...boxes.map(b => b.y)) <= 1);
    assert.ok(boxes.every(b => b.h === 32));
    await page.screenshot({ path: path.join(output, 'desktop.png') });
    checks.push('1920px: all controls share one row and 32px height');
    ...
    await expect(bar.locator('.advanced-inline')).toBeHidden();
    await bar.getByRole('button', { name: '更多筛选 (2)', exact: true }).click();
    ...
    await page.reload();
    await expect(bar.getByRole('button', { name: '更多筛选 (2)', exact: true })).toBeVisible();
    await bar.getByRole('button', { name: '重置', exact: true }).click();
    await expect(bar.getByRole('button', { name: '更多筛选', exact: true })).toBeVisible();
    await expand();
    await expect(page.getByRole('row').filter({ hasText: '客户待归口审核任务' })).toBeVisible();
    checks.push('1440px: advanced popup, active count, restored memory and reset');
