...
const OUTPUT = path.resolve(__dirname, '../../target/ui-audit/evidence');
...
async function main() {
  ...
  const result = { case: process.argv[2] || 'inspect', errors };
  try {
    assert(['inspect', 'cache', 'notices', 'deep-link', 'polling', 'badge'].includes(result.case), 'Unknown audit case');
    if (result.case === 'polling') await page.clock.install();
    await login(page, result.case === 'badge' ? '910003' : '910001', PASSWORD,
      result.case === 'deep-link' ? '/home/works?taskId=930002' : '');
    await page.getByText('审计专用A一级任务', { exact: true }).first().waitFor();
    if (result.case === 'inspect') {
      result.worksText = await page.locator('body').innerText();
      await page.goto(BASE + '/home/system/user?tab=security&action=password');
      await page.getByText('个人信息', { exact: true }).waitFor();
      result.userText = await page.locator('body').innerText();
      await page.screenshot({ path: path.join(OUTPUT, 'inspect-user.png'), fullPage: true });
    }
    if (result.case === 'cache') {
      ...
      try {
        ...
        await page.getByText('审计专用A一级任务', { exact: true }).waitFor({ timeout: 5000 });
        await page.locator('.works-container .el-loading-mask').waitFor({ state: 'hidden' });
        result.visibleUser = await page.locator('.user-name').innerText();
        result.otherUsersTaskVisible = true;
    ...
    if (result.case === 'notices') {
      ...
      assert.equal(firstPage.length, 15);
      ...
      assert.equal(result.readPaginationPages, 2);
    ...
    if (result.case === 'deep-link') {
      ...
      assert.equal(result.cachedVisitOpensTask, false);
      result.url = page.url();
    ...
    if (result.case === 'polling') {
      ...
      assert.equal(result.secondMountRequestsPerInterval, 2);
      await page.screenshot({ path: path.join(OUTPUT, 'polling-after-return.png'), fullPage: true });
    }
    if (result.case === 'badge') {
      await page.getByRole('menuitem', { name: /审核中心/ }).click();
      await page.getByText('审计手动绩效', { exact: true }).waitFor();
      await page.getByText('1 待处理', { exact: true }).waitFor();
      await page.locator('.audit-container .el-loading-mask').waitFor({ state: 'hidden' });
      result.pendingCountInAuditCenter = 1;
      result.sidebarBadgeCount = await page.getByRole('menuitem', { name: /审核中心/ }).locator('.el-badge').count();
      assert.equal(result.sidebarBadgeCount, 0);
      await page.screenshot({ path: path.join(OUTPUT, 'audit-count-missing-performance.png'), fullPage: true });
    }
