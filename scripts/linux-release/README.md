...
上传整个 `shuanggao-update-20260921` 文件夹到 `/root/biz-backend/`，不要覆盖旧包。本次开放双高办和管理员账号的工作纪实填报能力，并保留之前各项更新。

```bash
cd /root/biz-backend/shuanggao-update-20260921
chmod -R go-rwx .
...
本次步骤：文件校验 → 验证进程及空间 → 备份程序和前端 → 停止后端 → 完整备份当前数据库及 uploads → 补齐缺失结构 → 备份通知删除标记并清理失效提醒 → 启动后端并验证数据库连通 → 更新前端入口。
...
- 双高办（"双高"建设办公室，dept_id=100）全体有效账号及管理员（role=0）可填报工作纪实：新建、草稿、提交、刷新统计，口径为全校该年度全部三级任务，可填年度 2025—2029 且不超过当前月份。
- 双高办账号成为填报人后，可查看全部已提交纪实；不能删除纪实、无导出权限（导出仍仅管理员及 viewer-user-ids 配置账号）。
- 原"三级任务专业群审核人"填报体验与统计口径不变；无数据库结构变更，前端无改动。
- 保留此前版本全部能力：管理员工作纪实逻辑删除、同月重新填报；逐项添加改革任务及 Word 导出；强制改密读取线上 `force_password_change`，工号可登录及重置密码；失效审核通知清理。
